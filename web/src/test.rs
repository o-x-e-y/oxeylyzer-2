...
type Chars = Vec<(usize, (ReadSignal<String>, WriteSignal<String>))>;

#[component]
pub fn Test() -> impl IntoView {
    view! {
        <div style="margin-left: 10cqw">
            <Keys/>
        </div>
    }
}
...
fn Keys() -> impl IntoView {
    let (next_char_id, set_next_char_id) = create_signal(0);
    let (chars, set_chars) = create_signal::<Chars>(vec![]);

    for c in "abcdemore?????".chars() {
        let id = next_char_id.get();
        let sig = create_signal(c.to_string());

        set_chars.update(move |counters| counters.push((id, sig)));
        set_next_char_id.update(|id| *id += 1);
    }
    ...
    view! {
        <ul>
            <For
                ...
                children=move |
                    (_, (value, set_value)): (usize, (ReadSignal<String>, WriteSignal<String>))|
                {
                    view! { <Key value set_value/> }
                }
            />
...
#[component]
fn Key(value: ReadSignal<String>, set_value: WriteSignal<String>) -> impl IntoView {
    let input = move |ev| set_value.set(event_target_value(&ev));

    view! {
        <li>
            <input type="text" prop:value=value on:input=input/>
            <span>{value}</span>
        ...
        <div>
            ...
            <ul>
                <For
                    ...
                    key=|counter| counter.0
                    children=move |
                        (id, (value, set_value)): (usize, (ReadSignal<i32>, WriteSignal<i32>))|
                    {
                        view! { <Counter id value set_value/> }
                    }
                />
...
#[component]
fn Counter(id: usize, value: ReadSignal<i32>, set_value: WriteSignal<i32>) -> impl IntoView {
